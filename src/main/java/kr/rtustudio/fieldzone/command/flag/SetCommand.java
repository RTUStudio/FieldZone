...
public class SetCommand extends RSCommand<FieldZone> {

    private final RegionManager manager;

    public SetCommand(FieldZone plugin) {
    ...
    protected Result execute(CommandArgs data) {
        ...
        // /fz flag set <region> <flag> [true|false] — default: true
        boolean value = data.length() < 5 || Boolean.parseBoolean(data.get(4));

        FlagState current = region.hasFlag(flag);
        if (current != FlagState.NONE && current.toBoolean() == value) {
            notifier.announce(message.get(player, "region.flag.already-set")
            ...
            notifier.announce(message.get(player, "region.flag.set")
    ...
    protected List<String> tabComplete(CommandArgs data) {
        if (data.length() == 3) {
            ...
            return RegionFlag.values().stream().map(RegionFlag::getKey).toList();
