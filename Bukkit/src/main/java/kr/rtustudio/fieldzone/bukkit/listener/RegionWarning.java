...
public class RegionWarning extends RSListener<FieldZone> {
    ...
    private final GlobalConfig config; // config는 클래스 변수로 보관 (섹션 보관 금지)
    ...
    private void processPlayerWarning(Player player, PlayerWarningState state) {
        ...
        GlobalConfig.Warning warn = this.config.getWarning();
        GlobalConfig.Warning.Particle part = warn.getParticle();
